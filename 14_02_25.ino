...
double Kp = 28.0, Ki = 5.0, Kd = 10.0;  // Normal PID parameters
...
void controlSSRTask(void *pvParameters) {
  ...
  while (1) {
    ...
    if (elapsedControlCycleTime >= controlCycleInterval) {
      ...
      pwmValue = map(output, 0, 1000, 0, controlCycleInterval); // Fixed map() call
      pwmValue = constrain(pwmValue, 0, controlCycleInterval);  // Constrain PWM value
