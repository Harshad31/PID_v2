...
const long controlCycleInterval = 3000;  // Control cycle time (5000ms)
...
double Kp = 1.0, Ki = 0.2, Kd = 2.0;  // Normal PID parameters
...
void setup() {
  ...
  pidController.SetOutputLimits(0, 299720);  // Limit PID output to control SSR (0-255)
...
void controlSSRTask(void *pvParameters) {
  ...
  static unsigned long lastSSRUpdateTime = 0;  // To track SSR switching times
  static bool isSSROn = false;  // To track SSR state (ON/OFF)
  int pwmValue = 0;
  ...
  while (1) {
    ...
    if (millis() - lastControlCycleTime >= controlCycleInterval) {
      ...
      // Map PID output (0 - 299720) to Duty Cycle (0% - 100%)
      float dutyCycle = (output / 299720.0) * 100.0;  // Map to 0-100%

      // Print PID output and duty cycle
      ...
      // Map duty cycle to PWM control
      pwmValue = (dutyCycle / 100.0) * controlCycleInterval;  // Scale duty cycle to control cycle time
      pwmValue = constrain(pwmValue, 0, controlCycleInterval);  // Ensure PWM value is within limits

      // Now check whether to turn SSR on or off based on the calculated duty cycle
      if (millis() - lastSSRUpdateTime < pwmValue) {
      ...
      if (millis() - lastSSRUpdateTime >= controlCycleInterval) {
        lastSSRUpdateTime = millis();  // Reset for next cycle
      }
    }
  }
}
